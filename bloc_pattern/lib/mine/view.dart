...
class ViewState extends State with SignalApiView {
  UserDetail? _data;
  late MineCubit _cubit;
  @visibleForTesting
  static MineCubit? mockCubit;

  @override
  void initState() {
    _cubit = mockCubit != null ? mockCubit! : MineCubit();
    _cubit.getMineDetail();
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: [
        BlocConsumer(
          bloc: _cubit,
          listenWhen: (previous, current) => current is MineError,
          listener: (context, state) => error((state as MineError).e),
          buildWhen: (previous, current) => current is MineDone,
          builder: (context, state) {
            if (state is MineDone) {
              done(state.data);
            }
            return Column(
